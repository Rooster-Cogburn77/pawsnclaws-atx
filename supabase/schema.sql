-- PawsNClaws ATX Database Schema
...
CREATE TABLE donations (
  ...
  fee_covered BOOLEAN DEFAULT FALSE, -- donor covered processing fee
  donation_type TEXT DEFAULT 'one_time' CHECK (donation_type IN ('one_time', 'recurring', 'stock', 'crypto', 'daf')),
  payment_method TEXT, -- stripe, every_org, check, etc
  stripe_payment_id TEXT,
  stripe_subscription_id TEXT,
  donor_name TEXT, -- for anonymous or non-logged-in donors
  donor_email TEXT,
...
CREATE TABLE subscriptions (
  ...
  tier TEXT, -- colony_guardian, animal_hero, etc
  colony_id UUID REFERENCES colonies(id), -- if sponsoring a specific colony
  animal_id UUID REFERENCES animals(id), -- if sponsoring a specific animal
  created_at TIMESTAMPTZ DEFAULT NOW(),
...
CREATE TABLE sponsors (
  ...
  monthly_amount INTEGER, -- in cents
  stripe_subscription_id TEXT,
  benefits JSONB DEFAULT '{}', -- what they get at their tier
...
CREATE TABLE colonies (
  id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
  name TEXT NOT NULL,
  location_name TEXT, -- "Behind HEB on Riverside"
  latitude DECIMAL(10, 8),
  longitude DECIMAL(11, 8),
  description TEXT,
  cat_count INTEGER DEFAULT 0,
  all_tnr BOOLEAN DEFAULT FALSE, -- all cats TNR'd?
  caretaker_id UUID REFERENCES profiles(id),
  monthly_food_cost INTEGER, -- estimated in cents
...
CREATE TABLE deposit_assistance (
  ...
  monthly_repayment INTEGER, -- agreed monthly payment
...
CREATE TABLE surrender_prevention (
  ...
  contact_phone TEXT,
  pet_info TEXT,
  reason TEXT, -- why considering surrender
  assistance_needed TEXT, -- what help they need
...
CREATE TABLE food_stations (
  ...
  volunteer_id UUID REFERENCES profiles(id), -- who manages it
  is_active BOOLEAN DEFAULT TRUE,
  created_at TIMESTAMPTZ DEFAULT NOW()
);
...
CREATE TABLE volunteers (
  ...
  email TEXT NOT NULL,
  phone TEXT,
  skills JSONB DEFAULT '[]', -- ['tnr', 'transport', 'fostering', 'events']
  availability TEXT,
...
CREATE TABLE impact_metrics (
  ...
  vet_bills_paid INTEGER DEFAULT 0, -- in cents
